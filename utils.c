...
error:
	...
	fprintf(stderr, "Usage: SD%s [%sNUM] [%sNUM] <PATH>\n",
...
void print_header(FILE *f, const char *name)
{
	fprintf(f,
	"SD %s " SD_STR_VERSION "\n"
	"Copyright (C) 2014 Nate Stainton,\n"
    "Detects if a Flash Drive is using fake memory.\n"
	"Run with the path of a SD Card or Flash Drive to test.\n"
	"\n", name);
}
